...
static ssize_t fs_r_measure(struct file *filp, struct kobject *kobj, struct bin_attribute *bin_attr, char *buf, loff_t off, size_t count)
{
	...
	time_ns1=ktime_get_boottime_ns();
	time_ns2=ktime_get_boottime_ns();
	...
	while ( (time_ns2-time_ns1) < (1000000*MAX_MEASURE_TIME) ){ // wait to be ready for read new actual value
		do_lock(sl);
...
static int seahu_cd_add_slave_dynamic(struct w1_slave *sl){
	...
	*(p_to_p_groups+i)=NULL; // add end links of groups
	...
	// create sysfs group
	// left to create sysfs by standard way using "sl->family->fops->groups=p_to_p_groups" have problem with compile, can not write into constant
	// therefore I must do own call to create sysfs
	err = sysfs_create_groups(&sl->dev.kobj, p_to_p_groups);
	if (err) {
		dev_err(&sl->dev,
	...
	// sl->family->fops->groups=p_to_p_groups; // join syfs groups to w1 (problem compile) note upper
	return 0;
}
...
static void w1_seahu_cd_remove_slave(struct w1_slave *sl){
	...
	sysfs_remove_groups(&sl->dev.kobj, p_slave->p_to_p_groups);
...
static struct w1_family_ops w1_seahu_cd_fops = {
	...
	.groups		= NULL, // must be NULL because here .groups is static and can't be changed later by .add_slave function, therefore I create sysfs by myself in .add_slave function
};
