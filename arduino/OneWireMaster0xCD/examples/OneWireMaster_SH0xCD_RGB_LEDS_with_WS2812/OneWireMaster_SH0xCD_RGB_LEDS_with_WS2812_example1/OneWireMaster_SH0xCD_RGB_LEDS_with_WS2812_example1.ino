...
#define NUMPIXELS 60 // define number of pixels
#define ADD 5 // RBG step
#define DELAY 10 // delay between steps
...
void loop(void) {
  ...
  // chnage RED par of pixels color
  for (i=0; i<(255/ADD); i++){
  ...
  // chnage GREEN par of pixels color
  for (i=0; i<(255/ADD); i++){
    color = R*65536+G*256+B;
    set_color(color, ALL);
    G=G+ADD;
  ...
  // chnage BLUE par of pixels color
  for (i=0; i<(255/ADD); i++){
    color = R*65536+G*256+B;
    set_color(color, ALL);
    B=B+ADD;
