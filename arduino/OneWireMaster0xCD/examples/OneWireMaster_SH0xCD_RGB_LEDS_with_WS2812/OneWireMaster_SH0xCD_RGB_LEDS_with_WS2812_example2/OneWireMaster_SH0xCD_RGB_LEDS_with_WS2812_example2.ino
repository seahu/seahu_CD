...
//
// Exmaple description:
// ROTATION PIXELS COLOR ON RGB LED STRIP with WS2812 leds by 1-Wire bus with device "SEAHU 1-Wire controler for RGB LED STRIP with WS2812 leds"
// ----------------------------------------------------------------------------------------------------------------------------------------------
// Type device: 1-Wire
// Family code : 0xCD
...
OneWire  ds(10);  // on pin 10 (a 4.7K resistor is necessary) (basic 1-Wire library)
OneWireMaster0xCD du(&ds); // (extendet library for family dode 0xCD 1-Wire devices)
...
void loop(void) {
  ...
  for (i=1; i<NUMPIXELS+1; i++){
    set_actual_value(-1,SET_ROTATION); // select pixel position 0=all pixels
    delay(DELAY);
  }
}
