...
#
# Exmaple description:
# set fluent change color of frist color strip beetwen more colors (sone way can be control RGB2 or WHITE led strip)
# ------------------------------------------------------------------------------------------------------------------
#Type device: 1-Wire
# Family code : 0xCD
...
echo "500" > $DEVICE/section1/actual_value  # set 500ms time of fluent duration
echo "0" > $DEVICE/section1/actual_value  # 0=set no repeation change
echo $(color 255 0 0) > $DEVICE/section0/actual_value # set RED color
sleep 0.5
echo $(color 0 255 0) > $DEVICE/section0/actual_value # set GREEN color
sleep 0.5
echo $(color 0 0 255) > $DEVICE/section0/actual_value # set BLUE color
sleep 0.5
