/*
   ...
   White led <---------- A3  D3 2 -|2 PB3   PB2 7|- 7 D2 A1 (SLC,INT0) ----> 1-Wire
   Green led <--- (OC1B) A2 #D4 3 -|3 PB4   PB1 6|- 6 #D1   (MISO,OC0B,AIN1,OC1A)----> Red led
                            GND 4 -|4 GND   PB0 5|- 5 #D0   (MOSI,OC0A,AIN0,SDA,AREF)----> Blue led
                                   +-------------+               
*/

//  generate Device ID based on compile date and time
//#define __AVR_ATtiny85__
#define Timer1_OCR1A // select timmer1 with comparator A interrupt <-- best choice (no colision with PWM #D4, #D1, #D0)

#include "OneWireSlave0xCD.cpp.h"

#define CONT_OF_SECTIONS 5 // number of section (sensors or actors)
#define PIN_R 1
#define PIN_G 4
#define PIN_B 0
...
section sections[CONT_OF_SECTIONS]={
  ...
  { C_READ + C_WRITE +         + C_BOOL  , {.b=0}        , led             , NULL                   , false           , false },
  { C_READ + C_WRITE +         + C_BOOL  , {.b=0}        , Save            , NULL                   , false           , false },
};
typedef enum { RGBW, DURATION_RGBW, REPEAT_RGBW, LED, SAVE }; // enumerate of intem in section array for clear human read code
...
void pwm_step(){
  ...
  if (timer==0) {
    ...
    // set output to HIGH on start of new cycle except zero value  (but only if not set led indication who share same pin as pwm white. Identifu led has bigger priority then white pwm)
    if ( sections[LED].actualValue.b!=1 ) {
      if (W==0) digitalWrite(PIN_W, LOW);
      else digitalWrite(PIN_W, HIGH);
    }
    else digitalWrite(PIN_W, HIGH); // identidy led is set on
  }
  else {
    // set LOW value if raise to his boundary (but only if not set led indication who share same pin as pwm white. Identifu led has bigger priority then white pwm)
     if ( sections[LED].actualValue.b!=1) {
