...
const char counter_virt[] PROGMEM    = "Counter virtual counter";
const char info_display[] PROGMEM   = "String info display {1x32}";
...
void increase_couter(bool force_now); // tested measure function who increase virtual counter every run
 ...
 *        C_ALARM_STATUS - actual alarm status read only (setting is automatically, depends on alarm setting and actual value, reset by successfully reading actual value)
 *        C_MIN_ALARM    - on min. value alarm  (can be change by One Wire)
 *        C_MAX_ALARM    - on max. value alarm  (can be change by One Wire)
 *        
 *        C_MEMROY       - section is used only as 32B RAM memory, available through user note
...
section sections[CONT_OF_SECTIONS]={
  ...
  { C_READ +                   + C_U32BIT, {.u32=0}      , counter_virt    , NULL            , &increase_couter, false},
  { C_READ + C_WRITE           + C_U32BIT, {.u32=0}      , info_display    , display_buf     , NULL            , false}
};
typedef enum { COUNTER_A, COUNTER_B, COUNTER_C, COUNTER_OFF, INFO_DISPLAY }; // enumerate of intem in section array for clear human read code
...
void increase_couter(bool force_now){
  sections[4].actualValue.u32++;
}
